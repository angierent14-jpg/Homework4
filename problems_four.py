...
def problem1(searchstring):
    ...
    :return: string
    """
    pattern = r'^[a-zA-Z]{1,10}\.(?:[1-6][0-9]{2}|100|700)[a-zA-Z]*@(shield\.gov|starkindustries\.com)$'

    if re.fullmacth(pattern, searchstring):
        return 'valid'
    else:
        return 'invalid'
...
def problem2(searchstring):
    ...
    """
    #pass
    pattern = r'\b([A-Z][a-zA-Z]*(?: [A-Z][a-zA-Z]*)?) wrote (books|[A-Z0-9][a-zA-Z0-9]*(?: [A-Z][a-zA-Z0-9]*){0,2})\b'

    match = re.search(pattern, sentence)

    if match:
        author = match.group(1).strip(1)
...
def problem3(searchstring):
    ...
    """
    #pass
    pattern = r'\b([A-Z][a-zA-Z]*)\s+(Boy|Girl|boy|girl)\b'

    def repl(match):
        previous_word = match.group(1)
        target = match.group(2)
        if target.lower() == 'boy':
            replacement = 'Man' if target[0].isupper() else 'man'
        else:
            replacement = 'Woman' if target[0].isupper() else 'woman'
        return f"{previous_word} {replacement}"

    new_sentence, count = re.subn(pattern, repl, sentence)
